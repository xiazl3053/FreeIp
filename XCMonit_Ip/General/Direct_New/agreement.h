...
typedef struct AREA_INFO{
    ...
    union{
        ...
        UInt32    ulBlock[32];         /*最多32*32的块选区域, ulBlock[0]的第0位为1 表示从上到下第一行的左边第一个块选中*/
    }area;
...
typedef struct ARG_CMD{
    ...
    UInt32  ulBufferSize;      //  扩展内容大小(总大小等于 ulBufferSize*ulBufferNum)
	UInt32  ulBufferNum;       //  扩展内容个数.
...
typedef struct ARG_STREAM{
    ...
    unsigned char   ucRes[1];          //  保留
    UInt32   bMediaType:1;      //  A/V
    UInt32   bFrameType:5;      //  填写 (ARG_FRAME_TYPE)
    UInt32   bSize:22;          //  帧长度
    UInt32   bSubStream:2;      //  子码流
    UInt32   bRes:2;            //  保留
    ...
    UInt32   ulMagicNum;        //  用于验证头部正确 (数值为 ARG_STREAM_MAGIC_NUM 时正常)
}ARG_STREAM;
...
typedef struct TRANSPORT_HEAD_INFO{
    UInt32  ulTotleByte;                       /*总大小*/
    UInt32  ulFinByte;                         /*数据起点*/
    UInt32  ulId;                              /*标示*/
...
typedef struct VIDEOSTREAM_HEAD_INFO{
    ...
    UInt32  ucSerialNum;                      /*帧序列*/
    UInt32  ulTimeStamp;                      /*时间戳: 当前帧较第一帧的时间偏移量 单位为毫秒*/
...
typedef struct SEARCH_INFO{
    ...
    UInt32 ulWebPort;                         /*网页端口*/
    UInt32 ulRtspPort;                        /*rtsp端口*/
    UInt32 ulTcpPort;                         /*tcp端口*/

    UInt32 ulIP;                              /*IP地址*/
    UInt32 ulNetMash;                         /*子网掩码*/
    UInt32 ulGateway;                         /*网关*/
...
typedef struct USER_INFO{
    ...
    UInt32 ulPower;                           /*权限, 填写 USER_POWER_TYPE*/
...
typedef struct LOG_GREP_INFO{
    UInt32 ulLanguage;        /*mes语言*/

    UInt32 ulStartNo;         /*开始序号*/
    UInt32 ulNum;             /*数目*/

    UInt32 ulStartTime;       /*开始时间*/
...
typedef struct LOG_INFO{
	UInt32 totleNum;          /*日志文件总数*/
    UInt32 suitableNum;       /*匹配的日志文件总数*/

	UInt32 ulLanguage;        /*日志文件语言*/
	UInt32 ulEncodeType;      /*日志文件语言编码类型  0(UTF8)  1GB2312*/          
}LOG_INFO;
...
typedef struct LOG_EVENT_INFO{
    ...
    UInt32 ulType;            /*类型*/
    UInt32 ulCH;              /*通道*/
    UInt32 ulTIme;            /*返回自从 Unix 纪元（格林威治时间 1970 年 1 月 1 日 00:00:00）到当前时间的秒数.*/
...
typedef struct PLAYBACK_GREP_INFO{
    ...
    UInt32  ulStartNo;         /*开始序号*/
    UInt32  ulNum;             /*数目*/
    UInt32  ulStartTime;       /*录像开始时间*/
    UInt32  ulStopTime;        /*录像结束时间*/
    UInt32  ulMaxFileLen;      /*最大文件长度*/
    UInt32  ulMinFileLen;      /*最小文件长度*/

    UInt32  ulRes2[4];         /*保留2*/
}PLAYBACK_GREP_INFO;

typedef struct PLAYBACK_INFO{
    UInt32  totleNum;          /*录像文件总数*/
    UInt32  suitableNum;       /*匹配的录像文件总数*/
...
typedef struct PLAYBACK_FILE_INFO{
    ...
    UInt32  ulNo;              /*文件序号*/
    UInt32  ulStartTime;       /*开始时间*/
    UInt32  ulStopTime;        /*结束时间*/
    UInt32  ulFileLen;         /*文件大小: 单位字节*/
...
typedef struct WIFI_INFO{
	...
	UInt32 ulStrength;         /*0-10 10最强*/
    UInt32 ulIsScane;          /*0扫描得到,1手动添加*/
}WIFI_INFO;
...
typedef struct CHANNEL_SCOPE{
    ...
    UInt32 ulCHValidNum;             /*(ucCHScope)有效个数  0-64*/
}CHANNEL_SCOPE;
...
typedef struct MOTIONDETECTION_INFO{
    ...
    UInt32 ulTime;             /*报警时间 1970年1月1日00:00:00所经过的秒数*/
...
typedef struct PRESET_POINT_PROPERTY{
    ...
    UInt32      ulDwelltime;       /*停留时间: 单位秒数, 最大30分钟*/
...
typedef struct STORAGE_INFO{
    ...
    UInt32 ulTotle;             /*总容量: 单位兆(M)*/
    UInt32 ulFree;              /*可用空间: 单位兆(M)*/
...
typedef struct SMTP_PROPERTY{
    ...
    unsigned char ucRes1[2];                         /*保留1*/
    UInt32 ulPort;                            /*端口: 填写 25 或大于 1024*/
    UInt32 ulAnonymity;                       /*匿名 0匿名,1不匿名*/
    UInt32 ulEncryptionType;                  /*加密类型 0不加密,1SSL*/
    UInt32 ulTime;                            /*发送间隔时间 1-999*/
    UInt32 ulAccessory;                       /*是否加附件, 0加, 1不加*/
    UInt32 ulAccessoryType;                   /*附件类型(暂时没用)*/
...
typedef struct FTP_PROPERTY{
    ...
    unsigned char ucEnable;                          /*是否打开 0不需要, 1启动,*/
    unsigned char ucRes1[2];                         /*保留1*/
    UInt32 ulPort;                            /*端口: 填写 21 或大于 1024*/
    unsigned char ucUserName[ARG_USERNAME_LEN];      /*用户名*/
...
typedef struct DDNS_PROPERTY{
    ...
    UInt32 ulAlivePeriod;                     /*保活时间: 60-999999 单位秒*/
    UInt32 ulServiceType;                     /*服务类型: 0-普通用户 1-管理员*/
}DDNSSERVER_PROPERTY;
...
typedef struct LDC_PROPERTY{
    ...
    unsigned char  ucRes1[2];          /*保留*/
    Int32  lCenterXOffset;		       /*畸变中心点相对图象中心点水平偏移: -75到75*/
    Int32  lCenterYOffset;		       /*畸变中心点相对图象中心点垂直偏移: -75到75*/
    UInt32  ulRatio;            /*畸变程度: 0-511*/
    unsigned char  ucRes2[16];         /*保留*/
}LDC_PROPERTY;
