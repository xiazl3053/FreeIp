...
@interface ScanDeviceViewController()<UIImagePickerControllerDelegate,UINavigationControllerDelegate>
{
    int num;
    BOOL upOrdown;
    NSTimer * timer;
...
-(void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    [self dismissViewControllerAnimated:YES completion:^{}];
    NSString *strImg = [self findQRCode:image];
    if(strImg)
    {
        [[NSNotificationCenter defaultCenter] postNotificationName:NS_SCAN_DEVICE_NUMBER_VC object:strImg];
...
-(void)readerView:(ZBarReaderView *)readerView didReadSymbols:(ZBarSymbolSet *)symbols fromImage:(UIImage *)image
{
    ...
    //     }];
    DLog(@"%@", codeData);
    [[NSNotificationCenter defaultCenter] postNotificationName:NS_SCAN_DEVICE_NUMBER_VC object:codeData];
    [self dismissViewControllerAnimated:YES completion:nil];
}
